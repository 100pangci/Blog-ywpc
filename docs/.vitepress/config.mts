...
export default defineConfig({
  ...
  head: [
    ...
    ['script', {}, `document.addEventListener('DOMContentLoaded',()=>{const e=document.querySelector('.VPHero .image-container');if(e&&e.querySelector('img')){const t=e.querySelector('img');let o=!1;e.addEventListener('mousemove',n=>{const r=e.getBoundingClientRect(),s=(n.clientX-r.left)/r.width-.5,l=(n.clientY-r.top)/r.height-.5,i=Math.max(r.width,r.height);t.style.transform='translate('+Math.round(s*i*.02)+'px,'+Math.round(l*i*.02)+'px) rotateX('+Math.round(l*-8)+'deg) rotateY('+Math.round(s*8)+'deg) scale(1.02)';t.style.boxShadow='0 8px 50px rgba(233,82,149,0.4)';o||(o=!0,t.style.transition='none')});e.addEventListener('mouseleave',()=>{o=!1,t.style.transition='transform .6s ease,box-shadow .6s ease';t.style.transform='';t.style.boxShadow=''})}document.querySelectorAll('.VPFeature').forEach(c=>{c.addEventListener('mousemove',n=>{const r=c.getBoundingClientRect();c.style.setProperty('--mx',((n.clientX-r.left)/r.width*100)+'%');c.style.setProperty('--my',((n.clientY-r.top)/r.height*100)+'%')})})});`],
  ],
